...
export function getFeaturedWork(limit = 4) {
  ...
  // capped per category so one crowded category can't push the others out.
  const perCategory = Math.ceil(limit / WORK_CATEGORIES.length);
  return WORK_CATEGORIES.flatMap((c) =>
    pool.filter((i) => i.category === c.slug).slice(0, perCategory)
  ).slice(0, limit);
}
