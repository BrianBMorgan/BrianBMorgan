...
## Just shipped
- SEO/GEO pass. Every page now emits Open Graph + Twitter Card tags and a per-page canonical; a branded 1200x630 default OG card lives at `public/images/og-default.png` (work items use their own cover). JSON-LD structured data via `lib/seo.js` + `components/JsonLd.jsx`: a canonical Person + WebSite sitewide (root layout), CreativeWork on work items, BlogPosting on articles, ProfilePage on About, and BreadcrumbList on nested pages — all cross-referenced by stable @id. robots.txt already allows all crawlers (AI bots included); the sitemap already carried every route.
- brianbmorgan.com is live. The custom domain is configured in the repo's Pages settings (Actions-mode deploys ignore a CNAME file in the artifact — that's why the `public/CNAME` attempt didn't work; the file is removed). DNS was already pointing at GitHub; the HTTPS cert covers apex + www and enforcement is on. `site.url` now uses the domain, the `/BrianBMorgan` base path drops automatically via configure-pages, and the old github.io URLs redirect.
...
- `lib/site.js` facts are now real: contact email brian@sandbox-xm.com; canonical url initially pointed at the GitHub Pages address, since superseded by brianbmorgan.com (see above). Stats, availability, LinkedIn, and location confirmed accurate as-is.
